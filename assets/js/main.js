...
// the active slide's copy rides in behind the image reveal
function ot_hero1_content(swiper) {
	var slide = swiper.slides[swiper.activeIndex];
	if (!slide) return;
	...
	var ot_hero1_btns = Array.prototype.slice.call(content.querySelectorAll('.btn-elm > *'));
	...
	gsap.set(ot_hero1_targets, { clearProps: "opacity,transform" });

	gsap.timeline({ delay: .15 })
		.from(ot_hero1_author, {
			y: 24,
			opacity: 0,
			duration: .8,
			ease: "power3.out"
